...
describe('class `TreeUtils`', () => {
	...
	const getValue = keyPath =>
		utils.id(state, keyPath);

	describe('constructor', () => {
		it('accepts a custom `none` argument', () => {
			const customUtils = new TreeUtils(Seq.of('data'), 'id', 'childNodes', false);

			const noneValue = customUtils.find(state, node => node.get('name') === 'Not existing');
	...
	describe('method `find`', () => {
		...
		it('accepts an optional parameter `path` to restrict the haystack to a subtree.', () => {
			const keyPath = findKeyPathById('3', Seq.of('data', 'childNodes', 1, 'childNodes', 0));
	...
	describe('method `filter`', () => {
		const filterKeyPathByType = (type, seq) => {
			return utils.filter(state, (item) => {
		...
		it('accepts an optional parameter `path` to restrict the haystack to a subtree.', () => {
			const keyPaths = filterKeyPathByType('paragraph', Seq.of('data', 'childNodes', 1, 'childNodes', 0));
			expect(keyPaths.toArray().map(m => m.toJS())).toEqual([
				['data', 'childNodes', 1, 'childNodes', 0, 'childNodes', 0],
			]);
	...
	describe('method `byArbitrary`', () => {
		...
		it('returns the key path parameter if it is an array / a key path already.', () => {
			...
			expect(
				getValue(
					utils.byArbitrary(state, Seq(['data', 'childNodes', 1, 'childNodes', 0])).toJS(),
				),
			).toEqual('4');
		});
	});
