...
describe('class `TreeUtils`', function () {
	...
	var state = Immutable.fromJS(fixtures);
	...
	describe('constructor', function () {
		it('accepts a custom `none` argument', function () {
			var customUtils = new TreeUtils(Seq.of('data'), 'id', 'childNodes', false);
	...
	describe('method `find`', function () {
		var findKeyPathById = function findKeyPathById(id, seq) {
		...
		it('passes the current node\'s key path as second argument to the comparator', function () {
			var findSameKeyPath = function findSameKeyPath(seq) {
				return utils.find(state, function (node, keyPath) {
					return Immutable.is(seq, keyPath);
				});
			};
		...
		it('accepts an optional parameter `path` to restrict the haystack to a subtree.', function () {
			var keyPath = findKeyPathById('3', Seq.of('data', 'childNodes', 1, 'childNodes', 0));
			expect(keyPath).toBeUndefined();
		});
	});

	describe('method `filter`', function () {
		var filterKeyPathByType = function filterKeyPathByType(type, seq) {
		...
		it('returns a list of all key paths whose items passed the comparator function.', function () {
			...
			expect(keyPaths.toArray().map(function (m) {
				return m.toJS();
			})).toEqual([['data', 'childNodes', 0], ['data', 'childNodes', 1, 'childNodes', 0, 'childNodes', 0], ['data', 'childNodes', 1, 'childNodes', 1, 'childNodes', 0]]);
			expect(keyPaths.toArray().map(function (m) {
				return getValue(m.toJS());
			})).toEqual(['2', '5', '7']);

			var keyPaths2 = filterKeyPathByType('listItem');
			expect(keyPaths2.toArray().map(function (m) {
				return m.toJS();
			})).toEqual([['data', 'childNodes', 1, 'childNodes', 0], ['data', 'childNodes', 1, 'childNodes', 1]]);
			expect(keyPaths2.toArray().map(function (m) {
				return getValue(m.toJS());
			})).toEqual(['4', '6']);
		});
		...
		it('passes the current node\'s key path as second argument to the comparator', function () {
			var filterDescendants = function filterDescendants(seq) {
				return utils.filter(state, function (node, keyPath) {
					return Immutable.is(seq, keyPath.take(seq.size));
				});
			};
			expect(filterDescendants(Seq.of('data', 'childNodes', 1)).toJS()).toEqual([['data', 'childNodes', 1], ['data', 'childNodes', 1, 'childNodes', 0], ['data', 'childNodes', 1, 'childNodes', 1], ['data', 'childNodes', 1, 'childNodes', 0, 'childNodes', 0], ['data', 'childNodes', 1, 'childNodes', 1, 'childNodes', 0]]);
		});

		it('accepts an optional parameter `path` to restrict the haystack to a subtree.', function () {
			var keyPaths = filterKeyPathByType('paragraph', Seq.of('data', 'childNodes', 1, 'childNodes', 0));
			expect(keyPaths.toArray().map(function (m) {
				return m.toJS();
	...
	describe('method `byArbitrary`', function () {
		it('returns the key path representing an id.', function () {
			expect(utils.byArbitrary(state, '4').toJS()).toEqual(['data', 'childNodes', 1, 'childNodes', 0]);
		...
		it('returns the key path parameter if it is an array / a key path already.', function () {
			expect(utils.byArbitrary(state, Seq(['data', 'childNodes', 1, 'childNodes', 0])).toJS()).toEqual(['data', 'childNodes', 1, 'childNodes', 0]);
	...
	describe('method `ancestors`', function () {
		it('returns a list of all key paths that point at the ancestor nodes of the item at `id`.', function () {
			...
			expect(keyPaths.toArray().map(function (m) {
				return m.toJS();
			})).toEqual([['data', 'childNodes', 1, 'childNodes', 1], ['data', 'childNodes', 1], ['data']]);
	...
	describe('method `descendants`', function () {
		it('returns a list of all key paths that represent child nodes of the item at `id`', function () {
			...
			expect(keyPaths.toArray().map(function (m) {
				return m.toJS();
			})).toEqual([['data', 'childNodes', 1, 'childNodes', 0], ['data', 'childNodes', 1, 'childNodes', 1], ['data', 'childNodes', 1, 'childNodes', 0, 'childNodes', 0], ['data', 'childNodes', 1, 'childNodes', 1, 'childNodes', 0]]);
