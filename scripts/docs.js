...
var links = [
  {
    ...
    lookup: 'ImmutableJS',
    url:
      'http://github.com/lukasbuenger/immutable-treeutils/'
  },
  {
    name: 'ImmutableJS',
    lookup: 'ImmutableJS',
...
glob('index.js', {}, function(
  ...
  if (er) {
    ...
    var refs = []
    var parsed = fileNames.map(function(
      fileName
    ) {
      ...
      try {
        parse(data, {
          ...
          onComment: function(
            ...
            if (block) {
              var attributes
              var attributeMatch = tagExp.exec(
                text
              )
    ...
    var concatenatedComments = parsed.reduce(
      function(memo, item) {
