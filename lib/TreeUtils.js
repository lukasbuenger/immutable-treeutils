...
var TreeUtils = function () {
	function TreeUtils() {
		...
		var none = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : NONE;
  ...
  */
   ...
   */
		...
		value: regeneratorRuntime.mark(function nodes(state, path) {
			...
			return regeneratorRuntime.wrap(function nodes$(_context) {
				while (1) {
					switch (_context.prev = _context.next) {
						case 0:
							stack = _immutable.List.of(path || this.rootPath);
						...
						case 5:
							...
							childNodes = item.get(this.childNodesKey);
							...
							for (_iterator = item.get(this.childNodesKey).keys()[Symbol.iterator](); !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
								i = _step.value;
			...
			}, nodes, this, [[12, 16, 20, 28], [21,, 23, 27]]);
		})
   ...
   */
		...
		value: function byId(state, id) {
			...
			return this.find(state, function (item) {
				return item.get(_this.idKey) === id;
			});
		}
   ...
   */
		...
		value: function firstChild(state, idOrKeyPath) {
			var keyPath = this.byArbitrary(state, idOrKeyPath).concat([this.childNodesKey, 0]);
   ...
   */
		...
		value: function childAt(state, idOrKeyPath, index) {
			var keyPath = this.byArbitrary(state, idOrKeyPath).concat(this.childNodesKey, index);
			if (state.hasIn(keyPath)) {
				return keyPath;
			}
			return this.none;
   ...
   */
		...
		value: function descendants(state, idOrKeyPath) {
			...
			return this.filter(state, function (item) {
				return item.get(_this2.idKey) !== _this2.id(state, keyPath);
			}, keyPath);
		}
   ...
   */
		...
		value: function ancestors(state, idOrKeyPath) {
			...
			return this.byArbitrary(state, idOrKeyPath).reduceRight(function (memo, value, index, keyPath) {
				if ((index - _this3.rootPath.size) % 2 === 0 && index >= _this3.rootPath.size) {
   ...
   */
		...
		value: function position(state, idOrKeyPath) {
			...
			var order = this.byArbitrary(state, idOrKeyPath).reduceRight(function (memo, value, index) {
				if (index >= _this4.rootPath.size && index % 2 === 0) {
   ...
   */
		...
		value: function right(state, idOrKeyPath) {
			...
			var nextSiblingOfParent = void 0;
